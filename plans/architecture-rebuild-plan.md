...
> **For Hermes:** Execute this plan in ordered commits. Preserve the current public REST paths while moving code behind the new workspace boundaries.
...
openapi/openapi.json              # generated contract committed with code
```
...
2. `ADR-0006`: versioned SQLx migrations; `store::migrate()` bootstrap retired after baseline migration.
3. `ADR-0007`: auth/RBAC and API-token model; all control-plane endpoints protected by default.
4. `ADR-0008`: runner protocol and sandbox boundary; Docker socket is never exposed to the control-plane API container.
5. `ADR-0009`: artifact integrity/retention and backup ownership.
...
1. Implement password identity, Argon2id, JWT access/refresh rotation and protected browser routes.
2. Add project membership and RBAC policy checks in application services; require auth on all control-plane endpoints.
3. Activate API-token middleware with hashed lookup, scopes, expiry and last-used update.
4. Restrict CORS origins, add request IDs, rate limiting for auth and bounded request bodies.
5. Replace registry-only runners with a pull protocol: registration token, capabilities/tags, heartbeat timeout, job lease, acknowledgement, renewal and reconciliation.
6. Move execution to a separate runner process/service. Do not mount Docker socket into the API server. Inject decrypted secrets only into the runner execution environment; redact registered values from logs.
