...
- Embedded runner records job ownership in `job_leases` and reconciles expired leases. The external runner protocol MVP exposes register/heartbeat/poll/ack/renew/logs/complete with bearer runner credentials and lease tokens; `forge-runner` can run as a separate shell runner process and append stdout/stderr to attempt-owned logs. Durable queue, protocol secrets/artifacts, richer log chunks, Docker/Kubernetes isolation and sandbox hardening remain target work.
