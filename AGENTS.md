...
- **Назначение**: self-hosted CI/CD control plane. В MVP jobs выполняет embedded runner (Docker/shell) с `job_queue` + `job_leases` ledger/reconciliation; внешний runner protocol MVP покрывает register/heartbeat/poll/ack/renew/logs/complete, а `forge-runner` даёт отдельный shell-runner process поверх этого protocol. Production-grade dispatch policy, long-poll/wakeup, protocol secrets/artifacts, richer log chunks и sandbox hardening остаются target.
