...
Auth enforcement включается только если задан непустой `CICD_AUTH_SECRET`. Access token — JWT HS256 на 15 минут, содержит `sessions.id` и `users.token_version`, а protected API на каждом запросе проверяет активную session, enabled user, текущую роль и token version из БД. Refresh token хранится hash-ом в `sessions`, rotate-ится через `/auth/refresh` и отзывается через `/auth/logout`. Browser flow получает refresh token в `HttpOnly; SameSite=Lax` cookie `forge_refresh` и CSRF companion cookie `forge_csrf`; `/auth/refresh` и `/auth/logout` с cookie требуют совпадающий `X-CSRF-Token`. Rotation/reuse сериализуются по session family; повторное использование уже заменённого refresh token отзывает всю family и один раз инвалидирует ранее выданные access JWT через `users.token_version`. Body `refresh_token` остаётся совместимым для CLI/API clients и legacy dashboard migration. PAT `cicd_...` принимается как Bearer token при включённом enforcement; новые PAT в auth-mode требуют `project_id`, имеют явные scopes и срок действия.
...
Идемпотентно отзывает refresh session по переданному refresh token или по cookie `forge_refresh` + `X-CSRF-Token`. Session-bound access JWT с тем же `sessions.id` после logout перестаёт проходить protected API сразу; повторное использование уже заменённого refresh token приводит к `401`, отзывает всю `family_id` и один раз повышает `users.token_version`.
...
Для v1 `.forge-ci.yml` plan имеет `format: "v1-dag"`, `version: 1`, `parser_version: "forge-dsl/1.0.0"`, массив `jobs[]` с `key`, исходными `commands[]`, runtime `command` (`set -e` script), `needs[]`, `required_tags[]`, `required_secrets[]`, `artifact_paths[]`, resolved runtime stage/position и массив `dependencies[]` вида `{from,to}`. `defaults.tags` наследуется job-ами, `jobs.*.tags` заменяет defaults; tags сохраняются в `jobs.required_tags`/`job_queue.required_tags` и учитываются external runner claim. `jobs.*.secrets` сохраняется в `jobs.required_secrets`, попадает в `LeaseOffer.attempt.secrets` и ограничивает lease-scoped `secrets:resolve`. `jobs.*.artifacts.paths` сохраняется в `jobs.artifact_paths`, попадает в `LeaseOffer.attempt.artifacts`, embedded runner и external `forge-runner` собирают эти relative file paths и сохраняют metadata в `artifacts`. Unsupported v1 ключи (`on`, `retry`, `artifacts.expire_in`) пока отклоняются `400`, а не игнорируются.
...
> **Security note:** auth/RBAC enforcement включается только при непустом `CICD_AUTH_SECRET`. Без него все endpoints ниже работают в trusted-network режиме; с ним применяются JWT/PAT, scoped PAT, session-bound access invalidation, refresh cookie + CSRF для browser flow, session-family reuse revocation, route roles и project memberships для project-owned ресурсов. Tenant isolation, service-account tokens и scoped Git credentials ещё target.
...
### Rules, preferences, templates (stage 4)
...
| POST | `/api/v1/runner/heartbeat` | `{protocolVersion,status,draining,capacity,tags?,capabilities?,activeLeaseIds?}` → `204`; обновляет `last_seen_at`, capacity, capabilities и heartbeat snapshot; если `tags` переданы, заменяет stored tags, если нет — сохраняет текущие; optional `capabilities.executorKinds` валидируется как список executor names; stale online runner без unexpired active lease переводится maintenance loop-ом в `offline` после 120s |
| POST | `/api/v1/runner/work:poll` | `{protocolVersion,capacity,waitSeconds?,tags?,capabilityDigest?}` → `204` если совместимой работы нет или она временно недоступна из-за конкуренции claim, либо `200 LeaseOffer`; `waitSeconds` optional `0..30`, `0` сохраняет immediate poll, `>0` включает bounded long-poll |
...
| POST | `/api/v1/runner/leases/{lease_id}/renew` | `{protocolVersion,leaseToken,fencingToken}` → продлевает active lease и возвращает `{protocolVersion,leaseExpiresAt,renewAfter,cancelRequested}` |
...
| POST | `/api/v1/runner/leases/{lease_id}/artifact-sessions:begin` | runner Bearer credential + JSON `{protocolVersion, leaseToken, fencingToken, attemptId, artifactPath, artifactName, contentType?, declaredSize}` → resumable session `{sessionId, receivedBytes, highestChunk}`; idempotent per open `(attempt, artifactPath)` |
| POST | `/api/v1/runner/leases/{lease_id}/artifact-sessions/{session_id}/chunks` | runner Bearer credential + `X-Runner-Protocol-Version`, `X-Lease-Token`, `X-Fencing-Token`, `X-Attempt-Id`, `X-Chunk-Index`, `X-Chunk-Sha256` and raw body (≤8 MiB) → `{receivedBytes, highestChunk}`; strictly sequential high-water, replay or skip → 409, sha mismatch → 400 |
| POST | `/api/v1/runner/leases/{lease_id}/artifact-sessions/{session_id}/complete` | runner Bearer credential + JSON `{protocolVersion, leaseToken, fencingToken, attemptId, artifactSha256?}` → assembled `Artifact`; verifies declared byte count and optional whole-file sha256 |
| POST | `/api/v1/runner/leases/{lease_id}/artifact-sessions/{session_id}/abort` | runner Bearer credential + JSON `{protocolVersion, leaseToken, fencingToken, attemptId}`; validates the active acknowledged lease before marking the open session aborted |
...
| `/api/v1/runner/leases/{lease_id}/control` | Runner protocol |
