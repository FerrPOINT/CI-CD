...
| POST | `/api/v1/runner/work:poll` | `{protocolVersion,capacity,tags?,capabilityDigest?}` → `204` если совместимой работы нет или `200 LeaseOffer`; текущий MVP выполняет immediate poll без long-poll |
