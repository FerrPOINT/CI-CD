...
| POST | `/api/v1/runner/work:poll` | `{protocolVersion,capacity,waitSeconds?,tags?,capabilityDigest?}` → `204` если совместимой работы нет или `200 LeaseOffer`; `waitSeconds` optional `0..30`, `0` сохраняет immediate poll, `>0` включает bounded long-poll |
