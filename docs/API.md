...
Ограничения MVP: long-poll wakeup уже работает как process-local fast path + PostgreSQL `LISTEN/NOTIFY` для нескольких API-процессов, но это не broker/fairness policy; нет resumable/chunked artifact sessions, idempotent chunked log upload, pool/protected-tag policy, advanced capability matching, Docker/Kubernetes isolation, full secret rotation/redaction policy и production sandbox. Durable `job_queue`, bounded long-poll, ack-timeout requeue, basic tag + current `shell` executor matching, lease-scoped secret delivery, one-shot artifact upload, active runner heartbeat и stale-runner offline reconciliation уже есть как базовый dispatch ledger/execution boundary; production runner policy остаётся target в `docs/RUNNER_ARCHITECTURE.md` и `docs/contracts/RUNNER_PROTOCOL.md`.
