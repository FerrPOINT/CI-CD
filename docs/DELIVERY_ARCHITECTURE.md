...
| Quality | GitHub Actions запускает docs gate, backend fmt/clippy/workspace tests с real PostgreSQL integration и OpenAPI drift, а также frontend generated-client check/test/lint/build. Compose smoke, browser E2E/axe/performance и dependency/security scans остаются локальными проверками по необходимости. |
