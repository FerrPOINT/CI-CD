...
| Parser | `serde_yaml`-compatible API через maintained `yaml_serde`, durable `pipeline_plans` snapshot с parser version `forge-legacy-linear/1` или `forge-dsl/1.0.0`, config/plan SHA-256 | safe parser wrapper/implementation, line/column diagnostics, full policy-aware immutable plan |
