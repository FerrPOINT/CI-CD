...
| Job queue | ✅ MVP | `job_queue` материализует dispatch row на current queued attempt; trigger/retry/manual start enqueue-ят non-manual work, embedded и external runner claim берут работу через queue row + `SKIP LOCKED`, terminal/cancel/expiry закрывают row |
...
| External runner protocol + `forge-runner` | ✅ MVP | `/api/v1/runner/register`, heartbeat, immediate `work:poll`, ack/renew/logs/complete; credential и lease token хранятся только hash-ами; `work:poll` claim-ит durable `job_queue` row и отдаёт `workspace.checkoutUrl`; отдельный `forge-runner` shell process умеет checkout, renew, stdout/stderr log append и terminal completion; secrets/artifacts protocol, richer log chunks, Docker/Kubernetes sandbox и long-poll остаются target |
...
├── migrations/         # versioned SQLx migrations incl. 0018 job_queue
