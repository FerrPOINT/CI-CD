...
| External runner protocol + `forge-runner` | ✅ MVP | `/api/v1/runner/register`, heartbeat, immediate `work:poll`, ack/renew/logs/complete; credential и lease token хранятся только hash-ами; `work:poll` отдаёт `workspace.checkoutUrl`; отдельный `forge-runner` shell process умеет checkout, renew, stdout/stderr log append и terminal completion; durable `job_queue`, secrets/artifacts protocol, richer log chunks, Docker/Kubernetes sandbox и long-poll остаются target |
