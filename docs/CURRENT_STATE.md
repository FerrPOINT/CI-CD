...
| External runner protocol + `forge-runner` | ✅ MVP | `/api/v1/runner/register`, heartbeat, immediate `work:poll`, ack/renew/`secrets:resolve`/artifact upload/logs/complete; credential и lease token хранятся только hash-ами; `work:poll` claim-ит compatible durable `job_queue` row по `required_tags ⊆ runner.tags`, отдаёт `workspace.checkoutUrl`, declared `attempt.secrets` и `attempt.artifacts`; отдельный `forge-runner` shell process умеет checkout, scoped secret resolve после ack, renew, declared artifact upload, stdout/stderr log append с masking и terminal completion; richer log chunks, protected tags/pools, capability matching, Docker/Kubernetes sandbox и long-poll остаются target |
