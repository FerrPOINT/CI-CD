...
| External runner protocol + `forge-runner` | ✅ MVP | `/api/v1/runner/register`, heartbeat, immediate `work:poll`, ack/renew/`secrets:resolve`/logs/complete; credential и lease token хранятся только hash-ами; `work:poll` claim-ит compatible durable `job_queue` row по `required_tags ⊆ runner.tags`, отдаёт `workspace.checkoutUrl` и declared `attempt.secrets`; отдельный `forge-runner` shell process умеет checkout, scoped secret resolve после ack, renew, stdout/stderr log append с masking и terminal completion; artifacts protocol, richer log chunks, protected tags/pools, capability matching, Docker/Kubernetes sandbox и long-poll остаются target |
