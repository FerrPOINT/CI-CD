...
| Login UI | ✅ | `/login` вызывает auth API; Dashboard shell сначала восстанавливает access session через HttpOnly refresh cookie + CSRF companion cookie и включает redirect guard только при `401`; legacy `localStorage` refresh token используется один раз для migration и очищается |
...
| Error envelope + request_id | ✅ | Backend returns `{error:{code,message,request_id}}` + `x-request-id`; frontend transport maps structured envelopes, non-JSON HTTP failures, network failures and cancelled requests into typed `ApiError` while preserving status, `code`, `request_id`, safe details and `Retry-After` |
...
- Auth/RBAC пока без tenant isolation, service-account tokens, scoped Git credentials и session-family reuse detection; session-bound access invalidation, browser refresh cookie + CSRF, refresh rotate/logout/revoke, project membership, scoped PAT, configurable CORS allowlist и Git read/write checks реализованы как MVP-слой поверх глобальных ролей.
