...
| REQ-EXEC-002 | Внешний runner protocol и изолированные runner-ы | P1 | **Current verified MVP** | Control plane уже выдаёт работу через durable `job_queue` + runner protocol MVP: register/heartbeat/immediate poll/ack/`secrets:resolve`/renew/logs/complete, bearer runner credential, lease token, `workspace.checkoutUrl`, fencing generation, basic tag matching (`required_tags ⊆ runner.tags`) и scoped secret delivery. `forge-runner` даёт отдельный shell-runner process для checkout/command/secret env/stdout-stderr logs/terminal completion; production sandbox isolation, resource limits, pool/protected-tag policy, capability matching и artifacts protocol остаются target, чтобы API-процесс не был production-исполнителем пользовательского кода. |
