...
| REQ-EXEC-002 | Внешний runner protocol и изолированные runner-ы | P1 | **Current verified MVP** | Control plane уже выдаёт работу через runner protocol MVP: register/heartbeat/immediate poll/ack/renew/logs/complete, bearer runner credential, lease token, `workspace.checkoutUrl` и fencing generation. `forge-runner` даёт отдельный shell-runner process для checkout/command/stdout-stderr logs/terminal completion; durable queue, production sandbox isolation, resource limits и secrets/artifacts protocol остаются target, чтобы API-процесс не был production-исполнителем пользовательского кода. |
