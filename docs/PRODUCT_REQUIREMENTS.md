...
- **NFR-REL-05** Выполнение имеет явные timeout/cancel/recovery границы: current embedded runner хранит `job_leases` и fail-ит expired/missing owner; external runner protocol MVP добавляет heartbeat, lease token, ack/renew/control/artifact upload/logs/complete, `workspace.checkoutUrl`, fencing generation, basic tag + executor capability matching, active-lease heartbeat, stale-runner offline reconciliation и отдельный `forge-runner` shell process. Target дополнительно требует production dispatch, лимиты ресурсов, sandbox isolation, расширенную lost-runner restart/race suite и отсутствие бесконечного `running` в production topology.
