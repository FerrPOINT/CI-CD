...
| REQ-AUTH-001 | Identity, роли и API-токены | P0 | **Current verified** | Пользователи, роли, argon2id credentials, session-bound access JWT, refresh sessions с rotate/logout, session-family reuse revocation и scoped PAT хранятся и применяются при непустом `CICD_AUTH_SECRET`; без секрета действует trusted-network режим. |
| REQ-AUTH-002 | Project-scoped auth/RBAC enforcement | P1 | **Current verified MVP** | Project-owned API, name-based repo API и Git Smart HTTP read/write проверяют личность, route role, `project_memberships`, PAT `project_id` и scopes при непустом `CICD_AUTH_SECRET`; browser refresh/logout использует HttpOnly refresh cookie + CSRF proof. Tenant isolation, service-account tokens и scoped Git credentials остаются target. |
