...
| REQ-AUTH-001 | Identity, роли и API-токены | P0 | **Current verified** | Пользователи, роли, argon2id credentials, session-bound access JWT, refresh sessions с rotate/logout и PAT хранятся и применяются при непустом `CICD_AUTH_SECRET`; без секрета действует trusted-network режим. |
| REQ-AUTH-002 | Project-scoped auth/RBAC enforcement | P1 | **Current verified MVP** | Project-owned API проверяет личность, route role и `project_memberships` при непустом `CICD_AUTH_SECRET`. Tenant isolation, scoped PAT/SAT, Git binding и production cookie/CSRF/session-family policy остаются target. |
