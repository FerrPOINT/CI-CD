...
| REQ-PIPE-002 | План pipeline как неизменяемое доказательство | P1 | **Current verified MVP** | Перед запуском current legacy-конфигурация валидируется и фиксируется в `pipeline_plans` как immutable `legacy-linear` snapshot с raw config/fallback template, parser version, `config_sha256`, normalised plan JSON, `plan_sha256` и dependency edges между стадиями. Full v1 DAG (`version`, top-level `jobs`, `needs`), policy snapshot, variables в плане и parser diagnostics остаются target. |
...
- **NFR-REL-01** Pipeline plan, execution attempts, логи, metadata артефактов, deployment history и audit entries являются доказательствами и не переписываются задним числом; исправление создаёт новую запись. Current MVP закрывает immutable `pipeline_plans` snapshot, retry history для attempts/logs/artifact metadata и checksum для новых artifacts; full v1 DAG/policy plan и full storage lifecycle остаются target.
