...
- **NFR-REL-05** Выполнение имеет явные timeout/cancel/recovery границы: current embedded runner хранит `job_leases` и fail-ит expired/missing owner; external runner protocol MVP добавляет heartbeat, lease token, ack/renew/artifact upload/logs/complete, `workspace.checkoutUrl`, fencing generation, basic tag matching и отдельный `forge-runner` shell process. Target дополнительно требует production dispatch, лимиты ресурсов, sandbox isolation, lost-heartbeat suite и отсутствие бесконечного `running`.
