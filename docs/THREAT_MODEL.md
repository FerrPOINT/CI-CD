...
|---|---|---|---|
| Browser -> Dashboard nginx -> API (`/api`) | **S/E/I:** без `CICD_AUTH_SECRET` любой сетевой клиент получает API-доступ; текущие роли глобальные и не имеют project scope. **T/R:** часть изменяющих вызовов остаётся недостаточно связанной с project principal. **D:** per-process rate limit не защищает от распределённого flood. | Current: nginx proxy; SQLx parameter binding; request ID/error envelope; basic audit; conditional JWT/PAT enforcement и login rate limit при `CICD_AUTH_SECRET`. Target: TLS, CSRF для cookie, project RBAC и tenant/project filtering. Отсутствует: production default-deny, project membership, scoped PAT, production CORS allowlist. | `AUTHZ_CONTRACT` §1, §2--§7; route policy §6 |
...
|---:|---|---|---|
| P0 | **Auth/RBAC условны и не project-scoped.** При непустом `CICD_AUTH_SECRET` работают login/JWT/PAT и route-level global roles; без секрета API/Dashboard остаются trusted-network/open. Даже при включённом auth нет tenant/project membership, поэтому shared deployment не считается production-safe. | До shared-поставки: требовать непустой `CICD_AUTH_SECRET`, firewall/VPN и запрет публичной публикации API, Dashboard, Git и PostgreSQL. Затем завершить `AUTHZ_CONTRACT`: project membership, scoped PAT/SAT, deny-before-load, tenant/project filtering и route-policy tests. | Каждый route из `AUTHZ_CONTRACT` §6 возвращает 401/403/404 корректно; negative tests доказывают tenant isolation; production CORS/CSRF policy включена. |
