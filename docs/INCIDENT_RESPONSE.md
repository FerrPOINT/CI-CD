...
Текущий MVP использует embedded runner, durable `job_queue`, external runner protocol slice для register/heartbeat/poll/ack/renew/logs/complete и `forge-runner` shell process. Не заявляйте, что это уже production distributed runner platform: sandboxed runner, lost-runner auto-dispatch, inbound handlers и external notification adapters ещё не восстанавливают исполнение или доставку автоматически.
