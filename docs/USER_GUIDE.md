...
Страница репозитория открывает вкладку **Код** с деревом bare Git-репозитория. Вкладка, ветка и путь сохраняются в адресе страницы. Для файлов больше 512 KiB показывается ограниченный preview с предупреждением; бинарные файлы не отображаются.

Вкладка **Сравнение** принимает базовую и сравниваемую ветку или тег. Результат показывает изменения от общей базы к сравниваемой версии, статусы добавленных, изменённых и удалённых файлов, статистику строк и patch. Для бинарных файлов вместо числа строк показана пометка. Если сравнение не загрузилось, запрос можно повторить.

На вкладке **Релизы** можно создать metadata для тега, изменить название, описание и статус пре-релиза или удалить запись. Удаление записи не удаляет Git-тег.

Вкладка **Сравнение** принимает базовую и сравниваемую ветку или тег. Результат показывает изменения от общей базы к сравниваемой версии, статусы добавленных, изменённых и удалённых файлов, статистику строк и patch. Для бинарных файлов вместо числа строк показана пометка. Если сравнение не загрузилось, запрос можно повторить.

![Код репозитория](screenshots/09-repository-browser.png)

Список pull-запросов показывает номер, заголовок, статус, ветки, автора и дату. Доступны поиск по номеру, заголовку, веткам и автору, фильтр статуса и постраничная навигация. При создании укажите существующие ветки-источник и приёмник: форма не подставляет ветки автоматически и не позволяет выбрать одинаковые имена. Если подсказки ссылок репозитория недоступны, названия можно ввести вручную.

Страница pull-запроса показывает его детали и действия. Слияние и закрытие требуют подтверждения с номером PR и направлением веток; при ошибке запроса диалог остаётся открытым. «Посмотреть изменения» сохраняет контекст конкретного PR: номер, заголовок, ветки, merge-base, список файлов и patch. Возврат ведёт обратно в этот PR. При ошибке загрузки списка или сравнения доступен повтор запроса.
...
3. Проверьте список: доступны только метаданные, например key и timestamps. Поиск и страницы помогают найти нужный ключ; значение повторно не отображается.
4. Для замены выберите действие у ключа и введите новое значение. Для отзыва подтвердите удаление; диалог остаётся открытым при ошибке и позволяет повторить запрос.
...
2. Создайте запись с пяти-польным cron-выражением, Git ref и флагом включения, например `0 2 * * *` для `main`. Все cron-времена указаны в UTC.
3. Найдите запись поиском по cron или Git ref. Кнопка редактирования позволяет изменить параметры и приостановить запуски; удаление требует подтверждения, а при ошибке диалог остаётся открытым для повтора.
...
5. Если в строке показана приостановка из-за ошибки, раскройте причину и исправьте cron/ref через редактирование: `PATCH` очищает ошибку и пересчитывает `next_fire_at`.
...
5. На вкладке **История доставок** отфильтруйте записи по статусу и каналу. Страница и фильтры сохраняются в URL, поэтому выбранную выборку можно открыть прямой ссылкой; кнопки **Назад** и **Вперёд** позволяют пройти всю историю, а не только первые 20 записей.
6. Failed delivery можно явно поставить в повтор кнопкой **Повторить**; backend создаёт новую generation и не переписывает исходную историю. Нажмите на событие, чтобы открыть попытки доставки.
...
2. Просмотрите общее число запусков, успешные, неудачные и остальные статусы, долю успехов и среднюю длительность.
3. Отчёт охватывает всю историю проекта: общее число включает все статусы, доля успехов считается среди итоговых (`success`, `failed`, `canceled`), а средняя длительность - по запускам с известными временем начала и завершения. Из отчёта можно перейти к списку запусков.
4. Получите те же данные программно: `GET /api/v1/projects/{project_id}/reports/summary`.

Фильтры периода, графики динамики, percentiles, failure trends, DORA-агрегаты и export - **Target approved**. Не интерпретируйте текущую summary как отчёт за выбранный период: такого фильтра ещё нет.

![Отчёт проекта на тестовых данных](screenshots/2026-09-20-reports-ux/reports-filled-1920.png)
