...
- dependency audit (SQLx optional MySQL/RSA feature guard, `cargo audit --ignore RUSTSEC-2023-0071`, `pnpm audit --audit-level high`), committed-secret scan, SBOM drift, backend release build и Trivy image scan запускаются локально по release scope; deeper history/container secret scan, `cargo-deny` policy, broader image policy и release SBOM publication остаются target hardening.
