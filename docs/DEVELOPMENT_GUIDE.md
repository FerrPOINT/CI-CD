...
| CLI | `CICD_API_URL`, `CICD_API_TOKEN`, `CICD_TIMEOUT_SECONDS`, `CICD_OUTPUT` | `cicd-cli` вне контейнера |
...
| Dependency/SBOM security | Rust/Node advisories, committed-secret baseline и SBOM drift | Локально: SQLx optional MySQL/RSA feature guard, `cargo audit --ignore RUSTSEC-2023-0071`, `pnpm audit --audit-level high`, `python3 scripts/scan_secrets.py`, `python3 scripts/generate_sbom.py --check` | Local verification |
| Compose smoke | Запуск образов, `GET /api/v1/health`, `GET /api/v1/readiness` и frontend nginx | Локально: `just up && just health && just readiness` | Local verification |
...
| CLI + real API | Current smoke: project create/list, idempotent pipeline run, attempts, protected deployment approval, env/flag config precedence, bounded HTTP timeout, JWT/PAT auth-mode, RBAC denial, project-scoped read-only PAT, token redaction на CLI failure, JSON output и non-zero API error exit; target hardening: expanded redaction fixtures, OS keyring and request tracing; profiles/NDJSON/completions/stable exit codes are current | `cargo test -p cicd-cli --features integration --test cli_real_api -- --test-threads=1` с `CICD_TEST_DATABASE_URL`; CI backend job | Current verified MVP |
| Browser E2E | Critical UI journeys, all-route desktop accessibility и seeded performance smoke against built frontend и real API/PostgreSQL | Локально: `pnpm seed:evidence && pnpm e2e` на running Compose stack | Local verification; full auth/RBAC/CLI journeys остаются target |
| Accessibility | Keyboard flow, semantic controls, colour contrast и serious/critical violations | Локально: Playwright + axe по baseline routes; mobile drawer Escape/focus | Local verification; full keyboard/theme/Lighthouse coverage остаётся target |
| Performance | Seeded API read p95 и Dashboard route ready-time regression budgets | Локально: `frontend/e2e/performance.spec.ts` в `pnpm e2e`; Lighthouse/load/30-day SLO evidence target | Local verification |
...
`.github/workflows/ci.yml` запускается на `push` и `pull_request` в `main`, отменяет устаревшие in-progress runs для того же ref через concurrency и ограничивает jobs `docs`, `backend` и `frontend` через `timeout-minutes`.
...
| `frontend` | `pnpm install --no-frozen-lockfile`, `pnpm openapi:check`, `pnpm test -- --run`, `pnpm lint`, `pnpm build` |
| Local extended gate | Release build, CLI real-API smoke, Compose health smoke, Playwright/axe/performance, audits, secret scan, SBOM and image scan run locally when their scope is touched. |
...
В current workflow нет release build, CLI real-API smoke с auth/RBAC/PAT, OpenAPI backward compatibility diff, RU/EN i18n contract, Compose smoke, Playwright/axe/performance, dependency/security scans, а также isolated owner/runtime migration matrix, prior-schema upgrade, OpenAPI examples validation, Lighthouse, `cargo-deny`, history/container secret scans или coverage ratchet. Эти расширенные проверки выполняются локально по затронутому scope; часть из них остаётся target, когда требует ещё не поставленного контракта. Не утверждайте, что branch protection, approval policy или checks из устаревших narrative-доков фактически включены, пока это не подтверждено настройками GitHub.
...
| E2E/accessibility/performance | Current critical-flow/all-route axe gate плюс full auth/RBAC/CLI journeys, all-route keyboard coverage, theme axe matrix, Lighthouse budgets и retained reports |
| Security | current dependency audit/secret/SBOM drift gate с SQLx optional false-positive guard и critical container image scan плюс target `cargo-deny`, broader image policy и deeper history/container secret scan по согласованной severity policy |
