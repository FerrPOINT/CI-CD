...
Новый bare-репозиторий создаётся с символическим `HEAD` на `refs/heads/main`,
поэтому первый push в `main` сразу делает обычный clone работоспособным без
ручной настройки default branch на сервере.
