...
|---|---|---|
| External runner production boundary | credential rotation/revocation tables, richer artifact sessions, richer log chunks | `runners` + `job_queue` + `job_leases` уже покрывают protocol MVP: runner credential hash, heartbeat metadata, durable dispatch row с basic tag + current executor capability matching, lease token hash, `workspace.checkoutUrl`, ack-timeout requeue, ack/renew/control/`secrets:resolve`/artifact upload/logs/complete, fencing generation, stale offline reconciliation и shell `forge-runner` с active-lease heartbeat; target добавляет richer identity lifecycle, protected tags/pools, advanced capability matching, sandbox и chunked protocol data planes |
