...
| Webhooks | Сохраняются `url`, `events`, `enabled`, optional secret; terminal pipeline events создают `domain_events`/`outbox_messages`, worker выполняет basic retry/backoff, HMAC signing и пишет bounded delivery attempts/history. | Подписки с HMAC, версиями секретов, full delivery history, ограниченными повторными попытками, replay и dead-letter состояниями. |
...
| Наблюдаемость | `tracing`, HTTP TraceLayer, `/api/v1/health`, `/api/v1/readiness`, `/metrics` state gauges, opt-in OTLP and the Prometheus/Grafana/Alertmanager observability profile. | Queue/retry/delivery/scheduler-lag metrics, trace correlation and production alert-routing policy. |
...
2. Включить email fake/test adapter, затем Slack webhook. — частично: `slack_webhook`/`generic_webhook` доставляются через общий outbox delivery subsystem (см. docs/API.md Notifications); email adapter, rules/preferences/templates остаются target.
