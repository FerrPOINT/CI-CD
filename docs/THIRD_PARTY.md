...
6. Current CI gate: SQLx optional MySQL/RSA feature guard, `cargo audit --ignore RUSTSEC-2023-0071`, `pnpm audit --audit-level high`, `scripts/scan_secrets.py` и `scripts/generate_sbom.py --check`.
...
9. Перед релизом target: dependency update report, `cargo-deny`/license/source policy, container scan, deeper history/container secret scan и разбор accepted findings.
