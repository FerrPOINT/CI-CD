...
| [Trivy CLI](https://trivy.dev/) | Container image vulnerability scan для backend/frontend production images | Current security MVP | CI запускает pinned image digest через `scripts/scan_container_images.sh` и блокирует critical fixable CVE; broader severity/exception policy остаётся release hardening. |
