...
| [`cargo-audit`](https://crates.io/crates/cargo-audit) + [`cargo-deny`](https://github.com/embarkstudios/cargo-deny) | Dependency advisories, licenses, bans, duplicate crates, source policy | Security/release hardening | `cargo audit --ignore RUSTSEC-2023-0071` уже current CI gate вместе с SQLx optional MySQL/RSA feature guard; `git2 0.21.0` remediation ждёт совместимости с Rust 1.88 или MSRV bump; следующим шагом добавить `cargo-deny` с documented exceptions для license/source/ban policy. |
| [Trivy CLI](https://trivy.dev/) | Container image vulnerability scan для backend/frontend production images | Current security MVP | Локальный release gate запускает pinned image digest через `scripts/scan_container_images.sh` и блокирует critical fixable CVE; broader severity/exception policy остаётся release hardening. |
...
- `rust-version` workspace сейчас `1.88`; dependency upgrade, требующий больший MSRV, сначала меняет runtime policy и CI image.
