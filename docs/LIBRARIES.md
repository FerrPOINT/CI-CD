...
Для security/release проверки используются current SBOM refresh/drift gate, SQLx optional MySQL/RSA feature guard, `cargo audit --ignore RUSTSEC-2023-0071`, `pnpm audit --audit-level high`, `scripts/scan_secrets.py` и pinned Trivy critical container image scan через `scripts/scan_container_images.sh`; `cargo-deny`, broader image policy и deeper history/container secret scan добавляются по правилам [THIRD PARTY](THIRD_PARTY.md) и [DEVELOPMENT GUIDE](DEVELOPMENT_GUIDE.md#target-approved).
...
|---|---|---|---|
| Supply-chain gate | `cargo-deny`, broader image policy, history/container secret scan | Закрыть license/source/ban policy и image/secret exposure для RISK-008 | Добавлять после current feature guard, `cargo audit`, `pnpm audit --audit-level high`, baseline secret scan, SBOM drift gate и critical image scan; исключения фиксировать с owner/remediation. |
