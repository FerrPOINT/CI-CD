...
3. Сохранить исходный YAML, SHA-256 конфигурации, parser version и нормализованный execution plan.
...
На переходной фазе template `build/test/deploy` допустим только как явно выбранный `legacy_template` source. Отсутствие `.forge-ci.yml` не должно незаметно запускать deploy-template в production. Целевое поведение:
...
- Поддержать commit SHA pinning, parser diagnostics, immutable plan snapshot.
- Добавить `planned_jobs`/dependencies и DAG visualization API.
...
- Создать `forge-runner` binary и registration/auth protocol.
- Подключить heartbeat, capabilities, tags, capacity и drain.
