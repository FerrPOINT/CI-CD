...
| Pipeline plan | Trigger pipeline создаёт ровно один `pipeline_plans` snapshot на pipeline, replay `Idempotency-Key` возвращает тот же `plan_sha256`, raw config/template и normalised plan hashes сохраняются, прямой `UPDATE` snapshot отклоняется real-DB regression test-ом, а v1 `.forge-ci.yml` из bare repo сохраняет `v1-dag` jobs/needs/dependencies. Policy diagnostics, `on`/tags/retry/artifacts/secrets и job-level dispatcher требуют отдельной target suite. |
| Runner leases | Current embedded runner обязан создавать active `job_leases` вместе с claim, закрывать lease при success/failed/canceled, закрывать active lease при cancel pipeline и переводить expired/missing lease в failed attempt/job. Current external runner protocol MVP обязан регистрировать runner-а через bootstrap token, хранить credential/lease token только hash-ами, heartbeat-ить capacity/capabilities, выдавать queued work через `work:poll` вместе с `workspace.checkoutUrl`, принимать ack/renew/logs/complete и проверять fencing generation; `forge-runner` обязан иметь стабильный public CLI contract для запуска отдельного shell process и отправки stdout/stderr в `job_logs`. Target дополнительно требует lost-heartbeat/cancel-race suite, secrets/artifacts protocol, richer log chunks и sandbox/resource limits. |
...
| Secrets/artifacts | Создание/metadata допускаются только через safe contract; plaintext secret не возвращается API/UI; artifact download не читает `storage_path` вне `CICD_ARTIFACTS_DIR` и возвращает `409` при checksum drift для новых uploads. Current embedded runner secret injection и masking нуждаются в focused regression tests; target дополнительно проверяет least privilege lease, retention/scope и redaction во всех error/audit/trace каналах. |
| Rate limits | Current in-process middleware должен возвращать `429` до handler/auth для auth/API/Git/artifact route classes и не ограничивать health/readiness/openapi/metrics. Target дополнительно требует trusted proxy/distributed counters, per-account lockout, body/time/concurrency policy и нагрузочный evidence. |
