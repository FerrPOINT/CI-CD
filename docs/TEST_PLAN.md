...
| Auth и RBAC | Current enforcement требует `401`/`403`, enabled user, JWT/scoped PAT, PAT scopes/project boundary, session-bound access invalidation, browser refresh cookie + CSRF, refresh rotate/logout revoke, configurable CORS allowlist и audit event при заданном `CICD_AUTH_SECRET`; tenant isolation, service-account tokens, session-family reuse detection и persistent lockout остаются target. |
