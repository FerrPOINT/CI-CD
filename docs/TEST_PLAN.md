...
| Pipeline plan | Trigger pipeline создаёт ровно один `pipeline_plans` snapshot на pipeline, replay `Idempotency-Key` возвращает тот же `plan_sha256`, raw config/template и normalised plan hashes сохраняются, а прямой `UPDATE` snapshot отклоняется real-DB regression test-ом. Full v1 DAG/policy planner требует отдельной target suite. |
| Runner leases | Current embedded runner обязан создавать active `job_leases` вместе с claim, закрывать lease при success/failed/canceled, закрывать active lease при cancel pipeline и переводить expired/missing lease в failed attempt/job. Target external protocol дополнительно требует ack/renew, lease token, fencing rejection и lost-heartbeat suite. |
