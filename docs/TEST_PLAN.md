...
| API contract | Router, HTTP method/path, route-policy inventory, status, extractor и поведение без pool через `app(None)`, включая liveness `200` и readiness `503` без БД | `cargo test -p cicd-server --test api_contract`; inline `backend/src/authz.rs` OpenAPI/router coverage tests в `cargo test --workspace` | **Current verified** |
