...
| API contract | Router, HTTP method/path, route-policy inventory, status, extractor, safe error envelope и поведение без pool через `app(None)`, включая liveness `200` и readiness `503` без БД | `cargo test -p cicd-server --test api_contract`; inline `backend/src/api.rs`/`authz.rs` contract tests в `cargo test --workspace` | **Current verified** |
...
| CLI real API smoke | Настоящий `cicd-cli` против disposable HTTP API/PostgreSQL stack: config precedence, bounded HTTP timeout, JSON output, idempotent pipeline run, attempts, protected deployment approval, JWT/PAT auth-mode, RBAC denial, project-scoped read-only PAT, token redaction на failure и non-zero API error exit | Локально: `cargo test -p cicd-cli --features integration --test cli_real_api -- --test-threads=1` с `CICD_TEST_DATABASE_URL` | **Current verified local gate** |
...
| Dependency/SBOM security | Rust/Node advisories, dependency graph, committed-secret baseline и drift committed SBOM | Локально: SQLx optional MySQL/RSA feature guard, `cargo audit --ignore RUSTSEC-2023-0071`, `pnpm audit --audit-level high`, `scripts/scan_secrets.py`, `scripts/generate_sbom.py --check` | **Current verified local gate** |
| Compose smoke | Собранные образы, startup сервисов, `GET /api/v1/health`, `GET /api/v1/readiness` и frontend nginx | Локально: `docker compose config -q`, `docker compose up --build -d`, API/frontend smoke и `docker compose down -v --remove-orphans` | **Current verified local gate** |
...
| Playwright E2E | Критические пользовательские journeys, all-route desktop accessibility и seeded performance smoke на собранном frontend и real API/PostgreSQL stack | Локально: Docker Compose + `pnpm seed:evidence` + `pnpm e2e`; traces/screenshots/video сохраняются на failure | **Current verified local gate** |
| Accessibility | Keyboard journey, accessible names/roles, focus и серьёзные axe-нарушения | Локально: `frontend/e2e/accessibility.spec.ts`; mobile drawer Escape/focus в `critical-flows.spec.ts` | **Current verified local gate; target расширение** |
| Performance | Synthetic budgets для seeded API reads и Dashboard route ready-time; target Lighthouse/load/30-day SLO evidence | Локально: `frontend/e2e/performance.spec.ts` в `pnpm e2e` | **Current verified local gate; target расширение** |
...
| API contract без БД | `backend/tests/api_contract.rs`; `backend/src/api.rs` inline tests | Health возвращает `200`; readiness без pool возвращает `503`; project CRUD без pool возвращает `503`; пустой body PATCH отклоняется; raw SQLx details не возвращаются клиенту в `500 internal_error`. |
| Runtime config unit | `backend/src/config.rs` inline tests | `RuntimeConfig` проверяет required database URL для server startup, test/app defaults, normalization env-групп, допустимые bool aliases, `docker`/`host`, artifact TTL, queue timeout, base64 32-byte secrets key и invalid-value fail-fast. |
| AuthZ route inventory | `backend/src/authz.rs` inline tests | Executable `ROUTE_POLICIES` покрывает все generated OpenAPI operations и router path literals, не содержит duplicate method/path и не разрешает unpublished API route через user policy. |
...
| E2E Compose evidence | Локальный E2E gate использует disposable Compose stack с synthetic `CICD_SECRETS_KEY`, seeded repositories/projects/pipelines/artifacts и Playwright Chromium assertions. | **Current verified local gate** |
...
| Compose smoke | Изменены Dockerfile, Compose, image, startup/configuration или health/readiness route. | `docker compose config -q`, typed startup config validation, build, healthy startup, `GET /api/v1/health` и `GET /api/v1/readiness` успешны; временный stack остановлен. |
...
| Auth и RBAC | Current enforcement требует `401`/`403`, enabled user, JWT/scoped PAT, PAT scopes/project boundary, session-bound access invalidation, browser refresh cookie + CSRF, refresh rotate/logout revoke, session-family reuse revocation, configurable CORS allowlist и audit event при заданном `CICD_AUTH_SECRET`; tenant isolation, service-account tokens и persistent lockout остаются target. |
...
| Runtime configuration | Backend server config changes require typed parser tests for defaults, valid aliases and invalid values; `docker-compose.yml`, `.env.example`, `docs/ENV.md`, `docs/ARCHITECTURE.md` and `docs/CURRENT_STATE.md` must stay synchronized with `RuntimeConfig`. |
| Documentation drift | `scripts/verify_docs.py --all` сверяет route inventory, OpenAPI paths, screenshot manifest и его покрытие frontend routes, canonical names, current-state route count, readiness migration-version examples, frontend CI script references и архитектурную таблицу frontend stack с текущими источниками. |
...
`.github/workflows/ci.yml` запускается на `push` и `pull_request` в `main`, имеет concurrency per ref и bounded `timeout-minutes` на jobs `docs`, `backend` и `frontend`.

| Job | Обязательная проверка |
|---|---|
| `backend` | Rust + PostgreSQL 17 service: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo test --features integration --test integration_db -- --test-threads=1`, OpenAPI drift gate. |
| `frontend` | Node/pnpm: `pnpm install --no-frozen-lockfile`, `pnpm openapi:check`, `pnpm test -- --run`, `pnpm lint`, `pnpm build`. |
...
Current CI не запускает CLI real-API smoke, release build, OpenAPI backward compatibility, RU/EN i18n contract, Compose smoke, Playwright/axe/performance, dependency/security scans, isolated owner/runtime migration matrix, prior-schema upgrade, Lighthouse, OpenAPI examples validation, `cargo-deny`, history/container secret scan или coverage ratchet. Локальный extended gate выполняет реализованные проверки по scope; full auth/RBAC E2E, full keyboard-only/theme a11y, full locale E2E, load/30-day SLO evidence, Lighthouse budgets и coverage evidence остаются target.
