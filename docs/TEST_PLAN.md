...
| Auth и RBAC | Current enforcement требует `401`/`403`, enabled user, JWT/PAT, refresh rotate/logout revoke и audit event при заданном `CICD_AUTH_SECRET`; tenant isolation, scoped PAT, refresh-cookie/CSRF/session-family policy и persistent lockout остаются target. |
