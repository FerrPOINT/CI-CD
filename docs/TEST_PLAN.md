...
| Runner leases | Current embedded runner обязан создавать active `job_leases` вместе с claim, закрывать lease при success/failed/canceled, закрывать active lease при cancel pipeline и переводить expired/missing lease в failed attempt/job. Current external runner protocol MVP обязан регистрировать runner-а через bootstrap token, хранить credential/lease token только hash-ами, heartbeat-ить capacity/capabilities, выдавать queued work через `work:poll` вместе с `workspace.checkoutUrl`, принимать ack/renew/complete и проверять fencing generation; `forge-runner` обязан иметь стабильный public CLI contract для запуска отдельного shell process. Target дополнительно требует lost-heartbeat/cancel-race suite, secrets/logs/artifacts protocol и sandbox/resource limits. |
