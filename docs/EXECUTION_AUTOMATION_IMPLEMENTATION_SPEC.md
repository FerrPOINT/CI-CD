...
Этот документ описывает target-срез external runner/production scheduler/outbox. Current MVP уже имеет embedded runner, runner protocol register/heartbeat/bounded long-poll `work:poll`/ack/renew/control/`secrets:resolve`/artifact upload/logs/complete, basic tag + current executor capability matching, ack-timeout requeue, active runner heartbeat/stale offline reconciliation, `forge-runner` shell process, scheduler/outgoing webhook/local notification worker, bounded `outbox_delivery_attempts` history и explicit requeue failed delivery.

Уже реализовано в runner protocol MVP: env bootstrap registration token, heartbeat, durable `job_queue` claim, basic tag + current executor capability matching, bounded long-poll `work:poll` с `waitSeconds` `0..30` и wakeup через in-process signal + PostgreSQL `LISTEN/NOTIFY`, `workspace.checkoutUrl`, ack/renew/control/`secrets:resolve`/artifact upload/logs/complete, attempt/lease/log writes, fencing generation, ack-timeout requeue, stale-runner offline reconciliation без false-offline при unexpired active lease и отдельный shell runner binary с active-lease heartbeat. В target external-runner поставку остаются Docker-only/sandboxed runner, fairness, credential lifecycle, advanced pool/protected-tag/capability policy, lost-runner restart/race suite и production sandbox boundary.
