...
/// registers its dispatch signal in `main()`.
type NotifyFn = fn();

static NOTIFY_HOOK: std::sync::OnceLock<NotifyFn> = std::sync::OnceLock::new();

/// Register the runner-wakeup callback (idempotent, first call wins).
pub fn set_notify_hook(notify: NotifyFn) {
    let _ = NOTIFY_HOOK.set(notify);
}

fn notify_runner_work() {
    if let Some(notify) = NOTIFY_HOOK.get() {
        notify();
    }
}
