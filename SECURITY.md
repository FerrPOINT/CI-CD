...
- **Auth/RBAC условны**: если `CICD_AUTH_SECRET` не задан или пустой, API (`:22801`) и Dashboard (`:22802`) открыты в trusted-network режиме. Если секрет непустой, включаются login/JWT/PAT, session-bound access JWT, refresh cookie + CSRF, refresh rotate/logout/revoke, session-family reuse revocation, scoped PAT, coarse global roles, project membership RBAC, configurable CORS allowlist и Git Smart HTTP read/write checks по связанному проекту, но tenant isolation, service-account tokens, scoped Git credentials и production TLS/proxy defaults ещё не завершены.
