...
- Окружения и деплои: компактный список и история с поиском/страницами, явными ошибками и повторами; подтверждение approve/reject/rollback и предупреждение о каскадном удалении истории.
- Расписания проекта: компактный список с поиском и страницами, явное редактирование cron/ref и включения, видимая ошибка scheduler, повтор загрузки и подтверждение удаления до успешного ответа API.
- Секреты проекта: компактный список с поиском и пагинацией, явная замена значения, безопасное подтверждение удаления с ошибкой и повтором в диалоге.
