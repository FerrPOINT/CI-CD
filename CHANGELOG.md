...
- Окружения и деплои: компактный список и история с поиском/страницами,
  явными ошибками и повторами; подтверждение approve/reject/rollback и
  предупреждение о каскадном удалении истории. Убран занимающий первый экран
  MVP-callout, а toast использует контрастные семантические цвета темы.
- Расписания проекта: компактный список с поиском и страницами, явное
  редактирование cron/ref и включения, видимая ошибка scheduler, повтор
  загрузки и подтверждение удаления до успешного ответа API.
- Секреты проекта: компактный список с поиском и пагинацией, явная замена
  значения, безопасное подтверждение удаления с ошибкой и повтором в диалоге,
  адаптивная форма и доступные действия на узком экране.
- Внешняя выдача runner work соблюдает `max_running_jobs` и при конкурентных
  poll нескольких runner-ов; конфликт сериализации повторяется без двойного
  lease, а длительная конкуренция остаётся безопасным `204` для следующего poll.
=======
...
>>>>>>> 556de62 (fix(reports): clarify and compact project summary)
