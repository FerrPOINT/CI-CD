...
- Метаданные релизов отображаются компактным списком; создание и изменение
  разделены, а подтверждение удаления остаётся открытым до успешного ответа API.
- Браузер репозитория открывает код по умолчанию, сохраняет вкладку, ref и путь
  в URL, корректно показывает вложенные каталоги и файлы; ошибки загрузки
  изолированы по вкладкам, а ветки отделены от тегов.
