...
- Большие наборы Git refs и tags больше не захватываются и не рендерятся
  целиком: совместимые API-параметры включают потоковую пагинацию и поиск,
  вкладки хранят страницу в URL, а формы ограничивают debounced-подсказки.
- Каталоги репозитория используют bounded Git stdout, серверный поиск и
  URL-пагинацию по 100 записей вместо неограниченного API-ответа и DOM.
- Предпросмотр больших файлов читает из Git не больше 512 КиБ, сохраняет
  полный размер и остаётся доступным для клавиатурной прокрутки.
- Большие сравнения веток ограничивают текст patch первыми 512 КиБ и явно
  сообщают об усечении, сохраняя полный список изменённых файлов и line counts.
- История коммитов репозитория доступна дальше первых 50 записей и сохраняет
  выбранную страницу в URL.
- Длинные job-логи теперь открываются на последних строках, подгружают ранние
  записи без скачка позиции и не выталкивают пользователя вниз при live-обновлении.
- Журнал аудита больше не ограничивает поиск и навигацию последними 200
  событиями: добавлены серверная пагинация, точный action-фильтр, literal
  search, URL-состояние и отдельные empty/error/retry состояния.
- Каталог pull-запросов использует серверные поиск, фильтрацию и пагинацию,
  сохраняет состояние в URL и загружает детали PR отдельным запросом; подсказки
  source/target больше не смешивают ветки с тегами и служебными ссылками.
- Dashboard и каталог проектов сохраняют уже загруженные данные при частичном
  сетевом сбое и позволяют повторить загрузку projects, runs и runners локально,
  без полного обновления страницы.
- В деталях pull request бинарные файлы помечаются явно и больше не выглядят
  как текстовые изменения с выдуманными счётчиками `+0/-0`.
- Сохранение каналов уведомлений теперь атомарно: невалидный канал не удаляет
  действующие настройки. Форма сохраняет флаг активности и параметры политики,
  а при ошибке загрузки не позволяет отправить пустую замену. Статусы вебхуков
  и доставок читаются с достаточным контрастом в светлой и тёмной темах.

### Added

- Каталог артефактов job: компактный адаптивный список с поиском, фильтром
  состояния, страницами, копированием SHA-256 и явными ошибками скачивания.
- Каталог репозиториев Dashboard: компактные строки с постоянно видимыми
  действиями, поиск, фильтр по проекту, пагинация и понятные состояния ошибок.
- Список запусков CI/CD с серверной пагинацией и компактными строками; детали
  выполнения с автообновлением статусов и логов, состояниями ошибок и повторной
  загрузкой, подтверждением отмены и отдельным блоком диагностических действий.
...
- Общая оболочка Dashboard получила фиксированный header 60 px, sidebar 264 px,
  tablet rail 72 px и доступный mobile drawer с focus trap; вложенные маршруты
  корректно сохраняют активный раздел, а профиль и выход доступны в header.
- Страница доставки событий разделена на вкладки webhooks, доставок и уведомлений; списки и детали адаптированы для узких экранов, действия доступны с клавиатуры, ошибки загрузки и удаления можно повторить без потери контекста.
- История доставок получила серверную пагинацию, точный счётчик, URL-backed фильтры статуса и канала и прямые ссылки на выбранную страницу; legacy API-массив сохранён для совместимости.
- Окружения, расписания и секреты получили компактные списки с поиском, пагинацией, явными ошибками и подтверждениями рискованных действий.
- Отчёт проекта стал компактнее: сводка статусов, явный период и состав метрик, переход к запускам, состояния пустого результата и ошибки с повторной загрузкой.
- Pull-запросы: компактный список с поиском, статусным фильтром и пагинацией; подтверждение merge/close с контекстом веток и адаптивные детали.
- Метаданные релизов отображаются компактным списком; создание и изменение разделены, а подтверждение удаления остаётся открытым до успешного ответа API.
- Браузер репозитория открывает код по умолчанию, сохраняет вкладку, ref и путь в URL, корректно показывает вложенные каталоги и файлы; ошибки загрузки изолированы по вкладкам, а ветки отделены от тегов.
- Сравнение Git-версий: поля с явным направлением base/head и подсказками refs,
  корректные статусы добавленных/удалённых файлов и бинарных изменений,
  компактный адаптивный список файлов и повтор запроса после ошибки.
