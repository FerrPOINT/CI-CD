...
- Pull-запросы: компактный список с поиском, статусным фильтром и пагинацией;
  форма без вымышленных веток, подтверждение merge/close с контекстом веток,
  повтор загрузки после ошибок списка и diff, адаптивные детали.
